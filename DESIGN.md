...
The subtitle itself stays independent of the popup theme because it must remain readable over arbitrary video. It uses white text, a user-controlled near-black backing, two lines, balanced wrapping, safe-area spacing, and a restrained blur. Changing words use reduced opacity until Moonshine commits the line. The on-video status pill appears only when the phase or message changes, hides shortly after subtitles go live, and stays hidden while captions are flowing; the engine does not resend an unchanged status just because another inference pass finished.
